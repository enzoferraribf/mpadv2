...
export type PadWorkspaceViewEvent =
    ...
    | { kind: 'drawing-theme-preference-set'; preference: DrawingThemePreference }
    | { kind: 'layout-set'; layout: PadWorkspaceLayout }
    | { kind: 'sidebar-toggled' }

export function createPadWorkspaceViewState(preference: DrawingThemePreference = 'app'): PadWorkspaceViewState {
    return {
        activeTab: 'text',
...
export function reducePadWorkspaceViewState(state: PadWorkspaceViewState, event: PadWorkspaceViewEvent): PadWorkspaceViewState {
    switch (event.kind) {
        case 'tab-opened':
        ...
        case 'dialog-toggled':
            return { ...state, dialog: state.dialog === event.dialog ? null : event.dialog }
