...
test('renders standalone markdown images inline in the editor and reveals raw markdown while editing', async ({
    browser,
}) => {
    ...
    const page = await context.newPage()
    const imageUrl =
        "data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2064%2040'%3E%3Crect%20width='64'%20height='40'%20fill='%23c9a87c'/%3E%3C/svg%3E"
    const content = `alpha\n\n![badge](${imageUrl})\n\nomega`
    ...
    await expect(page.locator('[data-editor-image-widget="true"]')).toHaveCount(
        1,
    )
    await expect(
        page.locator('[data-editor-image-widget="true"] img'),
    ).toBeVisible()
    ...
    await expect(
        page.locator('.cm-line').filter({ hasText: '![badge](' }),
    ).toBeVisible()
...
test('shows persisted anime peer identity on remote text and drawing cursors', async ({
    browser,
}) => {
    ...
    await pageB.waitForFunction(
        () => (window as any).__mpad__?.getText() === '# badge test',
    )
    await expect(
        pageB
            .locator('.cm-ySelectionInfo')
            .filter({ hasText: 'Naruto Uzumaki' }),
    ).toBeVisible()
    await expect
        .poll(() =>
            pageA.evaluate(
                () =>
                    JSON.parse(window.localStorage.getItem('mpad.peer')!).name,
            ),
        )
        .toBe('Naruto Uzumaki')
    ...
    await pageB.waitForFunction(() => {
        ...
        return Array.from(appState.collaborators.values()).some(
            (value) =>
...
test('keeps remote text badges out of text replacement', async ({
    browser,
}) => {
    ...
    const sharedText = await pageB.evaluate(
        () => (window as any).__mpad__?.getText() ?? '',
    )
