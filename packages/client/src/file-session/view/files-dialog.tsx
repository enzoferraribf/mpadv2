                    ...
                    <CommandGroup heading={`Files (${input.files.length})`}>
                        <div className="dialog-scroll max-h-[36rem] overflow-y-auto px-2 pb-2">
                            {input.files.map((file) => (
                                <div key={file.meta.id} className="mb-3 flex items-center justify-between gap-4 rounded-md border border-[--stone-border] bg-[--stone-elevated] p-4">
                                    <div className="flex min-w-0 flex-1 items-start gap-3">
                                        <span className="command-icon-wrap"><HardDrive className="h-4 w-4" /></span>
                                    ...
                                    <div className="ml-4 flex gap-2">
                                        <Button
                                            onClick={() => input.onDownload(file)}
                                            variant="outline"
                                            size="sm"
                                            disabled={file.kind === 'transferring'}
