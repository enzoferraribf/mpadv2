...
export function TextDiffWorkspace(input: {
    ...
    const rightRevisionId = input.model.selection.rightSource.kind === 'revision'
        ? input.model.selection.rightSource.revisionId
        : null

    return (
        <section className="workspace-shell min-h-0" data-testid="workspace-shell">
            <ResizablePanelGroup autoSaveId="pad-diff-split" direction={input.direction} className="h-full min-h-0">
                <ResizablePanel defaultSize={24} minSize={18}>
                    <div className="diff-history-pane">
                        <div className="diff-history-header">Snapshots</div>
                        {input.model.loadingHistory ? <DiffEmpty label="Loading snapshots…" /> : null}
                        {!input.model.loadingHistory && input.model.historyError ? <DiffEmpty label={input.model.historyError} /> : null}
                        {!input.model.loadingHistory && !input.model.historyError && input.model.entries.length === 0 ? (
                            <DiffEmpty label="No snapshots yet. Wait for the next save." />
                        ) : null}
                        {!input.model.loadingHistory && !input.model.historyError && input.model.entries.length > 0 ? (
                            <div className="diff-history-list">
                                <div className={`diff-history-item diff-history-item-current${rightRevisionId === null ? ' active-right' : ''}`}>
                                    ...
                                    <div className="diff-history-item-actions">
                                        <button
                                            ...
                                            aria-pressed={rightRevisionId === null}
                                            className={`diff-history-toggle${rightRevisionId === null ? ' active' : ''}`}
                                            onClick={input.model.selectRightCurrent}
                                            type="button"
                                        >
                                            Right
                                        </button>
                                ...
                                {input.model.entries.map((entry) => (
                                    <div
                                        key={entry.id}
                                        className={readHistoryItemClassName(entry.id, leftRevisionId, rightRevisionId)}
                                    ...
                                    >
                                        ...
                                        <div className="diff-history-item-actions">
                                            <button
                                                ...
                                                aria-pressed={entry.id === leftRevisionId}
                                                className={`diff-history-toggle${entry.id === leftRevisionId ? ' active' : ''}`}
                                                disabled={!canSelectLeft(input.model.entries, entry.id, input.model.selection.rightSource)}
                                                onClick={() => input.model.selectLeftRevision(entry.id)}
                                                type="button"
                                            >
                                                Left
                                            </button>
                                            <button
                                                ...
                                                aria-pressed={rightRevisionId === entry.id}
                                                className={`diff-history-toggle${rightRevisionId === entry.id ? ' active' : ''}`}
                                                disabled={!canSelectRight(input.model.entries, leftRevisionId, entry.id)}
                                                onClick={() => input.model.selectRightRevision(entry.id)}
                                                type="button"
                                            >
                                                Right
                                            </button>
                                            <button
                                                ...
                                                aria-busy={input.model.pendingRevertRevisionId === entry.id}
                                                className="diff-history-toggle diff-history-revert diff-history-toggle-icon-only"
                                                disabled={input.model.pendingRevertRevisionId !== null}
                                                onClick={() => input.model.revertRevision(entry)}
                                                type="button"
                                            >
                                                {input.model.pendingRevertRevisionId === entry.id ? (
...
function DiffMainView(input: { state: PadHistoryWorkspaceModel['mainState'] }) {
    ...
    return <TextDiffMergeView leftContent={input.state.leftContent} rightContent={input.state.rightContent} />
}
...
function readEntry(entries: PadHistoryWorkspaceModel['entries'], revisionId: number | null) {
    if (revisionId === null) return null
    return entries.find((entry) => entry.id === revisionId) ?? null
}

function readHistoryItemClassName(entryId: number, leftRevisionId: number | null, rightRevisionId: number | null) {
