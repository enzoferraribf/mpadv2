...
import type { PadRoomSession } from './pad-room-types'

type UsePadRoomSessionInput<TKind extends PadRoomKind, TLocalState extends object> = {
    path: PadPath
    kind: TKind
...
export function usePadRoomSession<TKind extends PadRoomKind, TLocalState extends object>(
    input: UsePadRoomSessionInput<TKind, TLocalState>,
) {
    ...
    useEffect(() => {
        ...
        socket.onmessage = (event) => {
            if (!active) return
            const message = readServerRoomMessage(socketBytes(event.data))
...
function socketBytes(value: ArrayBuffer | Blob | string) {
