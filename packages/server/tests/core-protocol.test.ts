...
describe('core and protocol boundaries', () => {
    ...
    test('round-trips sync messages', () => {
        ...
        const encoded = encodeClientRoomMessage(createDocUpdateMessage(new Uint8Array([1, 2, 3])))
    ...
    test('rejects invalid room messages', () => {
        expect(() => readClientRoomMessage(new Uint8Array([99]))).toThrow('Unknown room message type: 99')
    })
