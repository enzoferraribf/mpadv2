...
describe('text comment overlay', () => {
    test('closes a thread overlay when that thread no longer exists', () => {
        const overlay: TextCommentOverlay = { kind: 'thread', threadId: 'thread-2' }
        const threads = [createThread({ id: 'thread-1' })]

        expect(closeMissingOverlayThread(overlay, threads)).toEqual({ kind: 'closed' })
    })
    ...
    test('builds anchored and detached markers with one active thread', () => {
        ...
        expect(buildCommentMarkers({
            activeThreadId: 'thread-c',
            anchors: new Map([
                ['thread-b', { top: 100, bottom: 124 }],
            ]),
            detachedGap: 20,
            detachedTop: 12,
            threads,
