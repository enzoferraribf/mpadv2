...
export async function openLiveFilesClient(ws: ServerWebSocket<WsData>) {
...
export async function closeLiveFilesClient(ws: ServerWebSocket<WsData>) {
...
export function applyLiveFilesClientMessage(ws: ServerWebSocket<WsData>, message: ClientRoomMessage) {
    const room = rooms.get(ws.data.roomName)
    if (!room) return
